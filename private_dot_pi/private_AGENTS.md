...
- Enable extensions with normal paths like `"extensions/native-notify.ts"`. A leading `-` (e.g. `"-extensions/native-notify.ts"`) is this repo's disable/exclude marker in the merged settings convention — do not use `-` to enable.
...
- `confirm-before-actions.ts` reads the Catppuccin theme file directly; preserve that unless intentionally redesigning.
...
- `~/.pi/agent/extensions/confirm-before-actions.ts` → `~/.local/share/chezmoi/private_dot_pi/private_agent/private_extensions/private_confirm-before-actions.ts`
