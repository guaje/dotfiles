...
- Enable extensions with normal paths like `"extensions/07-native-notify.ts"`. A leading `-` (e.g. `"-extensions/example.ts"`) is this repo's disable/exclude marker in the merged settings convention — do not use `-` to enable.
