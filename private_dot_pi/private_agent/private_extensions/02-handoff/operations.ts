...
import { shellLiteral, shellTest, sshExec, type SshTransportOptions } from "./transport.ts";
...
export function createRemoteOperations(options: RemoteOperationsOptions) {
  const access = async (path: string) => { await execute(options, shellTest("-r", remotePath(options, path))); };
  ...
  return {
    ...
    grep: { isDirectory: async (path: string) => { try { await execute(options, shellTest("-d", remotePath(options, path))); return true; } catch { return false; } }, readFile: async (path: string) => (await execute(options, `cat -- ${shellLiteral(remotePath(options, path))}`)).stdout.toString() } satisfies GrepOperations,
    find: { exists: async (path: string) => { try { await execute(options, shellTest("-e", remotePath(options, path))); return true; } catch { return false; } }, glob: async (pattern: string, cwd: string, opts: { limit: number }) => (await execute(options, `cd -- ${shellLiteral(remotePath(options, cwd))} && find . -path './${pattern.replace(/'/g, "'\"'\"'")}' -type f -maxdepth 10 2>/dev/null | head -n ${Math.max(1, opts.limit)}`)).stdout.toString().trim().split("\n").filter(Boolean).map((line) => line.replace(/^\.\//, "")) } satisfies FindOperations,
    ls: { exists: async (path: string) => { try { await execute(options, shellTest("-e", remotePath(options, path))); return true; } catch { return false; } }, stat: async (path: string) => { const target = remotePath(options, path); let isDir = false; try { await execute(options, shellTest("-d", target)); isDir = true; } catch { /* not a directory */ } return { isDirectory: () => isDir }; }, readdir: async (path: string) => (await execute(options, `ls -1 -- ${shellLiteral(remotePath(options, path))}`)).stdout.toString().trim().split("\n").filter(Boolean) } satisfies LsOperations,
