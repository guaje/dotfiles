...
export function sshExec(options: SshTransportOptions, script: string): Promise<TransportResult> {
  return new Promise((resolve, reject) => {
    ...
    const child: ChildProcess = (options.spawn ?? nodeSpawn)("ssh", args, { stdio: [input ? "pipe" : "ignore", "pipe", "pipe"], shell: false });
...
export function sshGetConfig(alias: string, spawn: typeof nodeSpawn = nodeSpawn, timeoutMs = SSH_TIMEOUT_MS): Promise<Record<string, string>> {
  return new Promise((resolve, reject) => {
    ...
    child.stdout?.on("data", (data: Buffer) => bounded(out, Buffer.from(data), MAX_OUTPUT_BYTES)); child.stderr?.on("data", (data: Buffer) => bounded(err, Buffer.from(data), MAX_OUTPUT_BYTES));
