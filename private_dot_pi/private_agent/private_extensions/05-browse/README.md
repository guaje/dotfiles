...
Non-secret configuration (provider base URLs, fallbackProviders, limits) is in Pi's settings under the `browse` key. Provider API keys are under `browse.providers` in `agent/credentials.json` (chmod 0600). `LINKUP_API_KEY` and `TAVILY_API_KEY` environment variables override the credentials file.
