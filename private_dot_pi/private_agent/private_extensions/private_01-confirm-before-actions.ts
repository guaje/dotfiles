...
import { registerHudItem, type HudItemHandle, type HudSegment } from "./00-hud/api.ts";
import { notifyPiWaitingForUser } from "./07-native-notify.ts";
import { getBashBackend, getBashTargetLabel } from "./02-handoff/backend-registry.ts";
...
const MANAGEMENT_STYLE_SUBMENU_SELECT_LIST_LAYOUT = { minPrimaryColumnWidth: 16, maxPrimaryColumnWidth: 24 };

let managingStyleHud: HudItemHandle | undefined;
let activeManagingStyleUi: { notify?: (message: string, level?: "info" | "warning" | "error" | "success") => void } | undefined;

export function getManagingStyleSegments(style: ManagingStyle): { full: HudSegment[]; compact: HudSegment[]; icon: HudSegment[] } {
  const icon = MANAGING_STYLE_STATUS_ICONS[style];
  const label = MANAGING_STYLE_STATUS_LABELS[style];
  const tone = style === "Empowerment" ? "success" : style === "Guidance" ? "warning" : "error";
  return {
    full: [{ text: icon, tone }, { text: ` ${label}`, tone: "muted" }],
    compact: [{ text: icon, tone }, { text: ` ${label.slice(0, 4)}`, tone: "muted" }],
    icon: [{ text: icon, tone }],
  };
}

// Kept as a stable call-site API; UI is no longer a footer/status dependency.
export function updateManagingStyleStatus(_ui: unknown, style: ManagingStyle) {
  managingStyleHud?.update({ variants: getManagingStyleSegments(style), visible: true });
}

function clearManagingStyleStatus() { managingStyleHud?.dispose(); managingStyleHud = undefined; }
...
function patchBuiltInSettingsMenu(): Promise<void> {
  if (!settingsSelectorPatchPromise) {
    settingsSelectorPatchPromise = (async () => {
      const [interactiveModeModule, settingsSelectorModule, themeModule] = await Promise.all([
      ...
      if (!InteractiveMode.prototype.__managingStyleUiPatched && InteractiveMode.prototype.showSettingsSelector) {
        ...
        InteractiveMode.prototype.showSettingsSelector = function showSettingsSelector(this: { ui?: { notify?: (message: string, level?: "info" | "warning" | "error" | "success") => void } }, ...args: unknown[]) {
      ...
      if (!InteractiveMode.prototype.__managingStyleShortcutPatched && InteractiveMode.prototype.setupExtensionShortcuts) {
        ...
        InteractiveMode.prototype.setupExtensionShortcuts = function setupExtensionShortcuts(this: { defaultEditor?: { onExtensionShortcut?: (data: string) => boolean }; createExtensionUIContext?: () => { notify?: (message: string, level?: "info" | "warning" | "error" | "success") => void } }, ...args: unknown[]) {
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    managingStyleHud?.dispose();
    managingStyleHud = registerHudItem({ owner: "confirm-before-actions", id: "management-style", zone: "modeRight", order: 100, importance: "required", variants: getManagingStyleSegments(await refreshManagingStyleCache()) });
  });

  pi.on("session_shutdown", (_event, ctx) => {
    clearManagingStyleStatus();
    sessionManagingStyle = undefined;
  ...
  pi.registerShortcut?.(MANAGEMENT_STYLE_CYCLE_SHORTCUT, {
    ...
    handler: async (ctx: { ui?: { notify?: (message: string, level?: "info" | "warning" | "error" | "success") => void } }) => {
  ...
  pi.registerTool({
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      const operations = getBashBackend();
      return createBashTool(ctx.cwd, operations ? { operations } : undefined).execute(toolCallId, params, signal, onUpdate, ctx);
    },
  ...
  pi.on("tool_call", async (event, ctx) => {
    if (isToolCallEventType("bash", event)) {
      ...
      const ok = await confirmWithWorkingHidden(
        ...
        `${summarizeBash(event.input.command)}${getBashTargetLabel() ? `\n\n${uiLabel("Remote target:")} ${getBashTargetLabel()}` : ""}`,
      );
