 ...
 *
 * Purely presentational: `renderSubagentCall` draws the tool-call preview and
 * `renderSubagentResult` draws the completed/expanded result. All formatting
 * helpers (tokens, usage, model label, tool-call preview) live here too.
 *
 * Depends on result.ts (data extraction) and types.ts only — no subprocess,
 * no orchestration. This keeps the drawing logic independently testable and
 * out of the entry module.
 */

import * as os from "node:os";
import type { AgentToolResult } from "@earendil-works/pi-agent-core";
import { getMarkdownTheme } from "@earendil-works/pi-coding-agent";
import { Container, type Component, Markdown, Spacer, Text } from "@earendil-works/pi-tui";
...
/** Build the model label shown in usage stats, appending the thinking level when set. */
export function modelDisplayFor(r: SingleResult): string | undefined {
...
/** Render the completed (or streaming) tool result, collapsed or expanded. */
export function renderSubagentResult(
	...
	if (details.mode === "single" && details.results.length === 1) {
		...
		if (expanded) {
			...
			const usageStr = formatUsageStats(r.usage, modelDisplayFor(r));
			if (usageStr) {
				container.addChild(new Spacer(1));
	...
	if (details.mode === "chain") {
		...
		if (expanded) {
			...
			for (const r of details.results) {
				...
				const stepUsage = formatUsageStats(r.usage, modelDisplayFor(r));
	...
	if (details.mode === "parallel") {
		...
		if (expanded && !isRunning) {
			...
			for (const r of details.results) {
				...
				const taskUsage = formatUsageStats(r.usage, modelDisplayFor(r));
