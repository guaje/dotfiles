/**
 * Subprocess execution for the subagent extension.
 *
 * `runSingleAgent` is the single point that:
 *   - resolves an agent's model (explicit frontmatter override, or automatic
 *     selection via model-selection.ts),
 *   - spawns an isolated child `pi` process in JSON mode,
 *   - streams its messages/usage into a SingleResult,
 *   - cleans up temp files.
 *
 * This is the only subagent module with real side effects (subprocess + fs).
 */
...
/** Resolve the `pi` binary + args to invoke, re-entering the current script when possible. */
...
export async function runSingleAgent(
	...
	// Model selection. An explicit model: frontmatter is a hard override.
	// Otherwise pick a model (+ thinking level) for the child process.
